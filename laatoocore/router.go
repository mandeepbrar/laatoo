...
func (router *Router) Group(ctx core.Context, path string, conf map[string]interface{}) core.Router {
	...
	if !bypassauth {
		permission, permok := conf[core.CONF_PERMISSION]
		if permok {
			retRouter.Use(ctx, func(permCtx core.Context) error {
				if retRouter.environment.IsAllowed(permCtx, permission.(string)) {
					return nil
				} else {
					return permCtx.NoContent(http.StatusForbidden)
				}
			})
		}
	}
...
func (router *Router) Get(ctx core.Context, path string, conf map[string]interface{}, handler core.HandlerFunc) error {
	router.eRouter.Get(path, func(pathCtx *echo.Context) error {
		...
		if conf != nil {
			perm, permok := conf[core.CONF_PERMISSION]
			if permok {
				if !router.environment.IsAllowed(corectx, perm.(string)) {
					return pathCtx.NoContent(http.StatusForbidden)
				}
			}
		}
...
func (router *Router) ServeFile(ctx core.Context, pagePath string, conf map[string]interface{}, dest string) {
	if conf != nil {
		perm, permok := conf[core.CONF_PERMISSION]
		if permok {
			if !router.environment.IsAllowed(ctx, perm.(string)) {
