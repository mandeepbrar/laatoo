...
import (
	...
	"path/filepath"
	"strings"
	modulessdk "modulesbase/sdk"

	"github.com/mholt/archiver"
)

func processArchive(ctx core.RequestContext, mod, file string, repositoryFiles components.StorageComponent) (*modulessdk.ModuleDefinition, error) {
...
func readMod(ctx core.RequestContext, modName string) (*modulessdk.ModuleDefinition, error) {
	...
	mod := &modulessdk.ModuleDefinition{Name: modName}
	mod.SetId(modName)
...
func readConf(ctx core.RequestContext, mod *modulessdk.ModuleDefinition, conf config.Config) error {
...
func readDependencies(ctx core.RequestContext, mod *modulessdk.ModuleDefinition, conf config.Config) error {
...
func readParams(ctx core.RequestContext, mod *modulessdk.ModuleDefinition, conf config.Config) error {
	params := make([]modulessdk.ModuleParam, 0)
	...
	if ok {
		...
		for _, paramName := range paramNames {
			...
			modParam := modulessdk.ModuleParam{Name: paramName, Type: ptype, Description: desc}
...
func readObjects(ctx core.RequestContext, mod *modulessdk.ModuleDefinition, conf config.Config) error {
...
func readModObj(ctx core.RequestContext, objName string, mod *modulessdk.ModuleDefinition, conf config.Config) error {
...
func readSubModules(ctx core.RequestContext, mod *modulessdk.ModuleDefinition, conf config.Config) error {
...
func writeParamsForm(ctx core.RequestContext, mod *modulessdk.ModuleDefinition) ([]byte, error) {
