...
func ProcessArchive(ctx core.RequestContext, mod, file, tmppath string, repositoryFiles components.StorageComponent) (*ModuleDefinition, error) {
	err := extractArchive(ctx, mod, file, tmppath, repositoryFiles)
	if err != nil {
		return nil, errors.WrapError(ctx, err)
	}
	return readMod(ctx, mod, tmppath)
}
...
func readMod(ctx core.RequestContext, modName, tmppath string) (*ModuleDefinition, error) {
...
func readParams(ctx core.RequestContext, mod *ModuleDefinition, conf config.Config) error {
	...
	if ok {
		...
		for _, paramName := range paramNames {
			...
			modParam := Param{Name: paramName, Type: ptype, Description: desc}
...
func readModObj(ctx core.RequestContext, objName string, mod *ModuleDefinition, conf config.Config) error {
	...
	if modObj != nil {
		...
		if inf != nil {
			...
			if ok {
				svcsArr := make([]Service, len(svcsConf))
			...
			if ok {
				facsArr := make([]Factory, len(facsConf))
			...
			if ok {
				chnsArr := make([]Channel, len(chansConf))
...
func readServices(ctx core.RequestContext, modDir string, conf config.Config) ([]Service, error) {
	...
	elems := make([]Service, len(elemConfs))
...
func readFactories(ctx core.RequestContext, modDir string, conf config.Config) ([]Factory, error) {
	...
	elems := make([]Factory, len(elemConfs))
...
func readChannels(ctx core.RequestContext, modDir string, conf config.Config) ([]Channel, error) {
	...
	elems := make([]Channel, len(elemConfs))
...
func readEngines(ctx core.RequestContext, modDir string, conf config.Config) ([]Engine, error) {
	elemConfs, elemNames, err := readElementConfs(ctx, modDir, "engines")
	if err != nil {
		return nil, errors.WrapError(ctx, err)
	}

	elems := make([]Engine, len(elemConfs))
...
func readRules(ctx core.RequestContext, modDir string, conf config.Config) ([]Rule, error) {
	elemConfs, elemNames, err := readElementConfs(ctx, modDir, "rules")
	if err != nil {
		return nil, errors.WrapError(ctx, err)
	}

	elems := make([]Rule, len(elemConfs))
...
func readTasks(ctx core.RequestContext, modDir string, conf config.Config) ([]Task, error) {
	elemConfs, elemNames, err := readElementConfs(ctx, modDir, "tasks")
	if err != nil {
		return nil, errors.WrapError(ctx, err)
	}

	elems := make([]Task, len(elemConfs))
...
func readSubModules(ctx core.RequestContext, mod *ModuleDefinition, conf config.Config, tmppath string) error {
...
func readServiceConf(ctx core.RequestContext, name string, conf config.Config) (Service, error) {
	return Service{}, nil
}

func readFactoryConf(ctx core.RequestContext, name string, conf config.Config) (Factory, error) {
	return Factory{}, nil
}

func readChannelConf(ctx core.RequestContext, name string, conf config.Config) (Channel, error) {
	return Channel{}, nil
}

func readRuleConf(ctx core.RequestContext, name string, conf config.Config) (Rule, error) {
	return Rule{}, nil
}

func readTaskConf(ctx core.RequestContext, name string, conf config.Config) (Task, error) {
	return Task{}, nil
}

func readEngineConf(ctx core.RequestContext, name string, conf config.Config) (Engine, error) {
	return Engine{}, nil
}

func WriteConfigForm(ctx core.RequestContext, mod *ModuleDefinition) ([]byte, error) {
