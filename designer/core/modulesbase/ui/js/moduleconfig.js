...
class ModuleConfig extends React.Component {
    ...
    loadModuleForm=(moduleId)=> {
        if(moduleId != "") {
            let req = RequestBuilder.URLParamsRequest({id: "Module_"+moduleId})
            console.log("load module form request", req, Application)
            let prom = DataSource.ExecuteService("module_config", req, {})
