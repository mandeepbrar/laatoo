...
type ModuleDesignServerRule struct {
	dataStore data.DataComponent
}

func (rule *ModuleDesignServerRule) Initialize(ctx ctx.Context, conf config.Config) error {
	dataSvcName := "dataadapter.dataservice.ModuleDesignServer"
...
func (rule *ModuleDesignServerRule) Condition(ctx core.RequestContext, trigger *rules.Trigger) bool {
...
func (rule *ModuleDesignServerRule) Action(ctx core.RequestContext, trigger *rules.Trigger) error {
	ent, _ := trigger.Message.(*modulesrepository.Entitlement)
	if ent.Local == true {
		mod := &ModuleDesignServer{Name: ent.Name}
		mod.SetId(ent.Name)
