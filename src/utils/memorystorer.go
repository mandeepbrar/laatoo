...
func NewMemoryStorer() *MemoryStorer {
	memStor := &MemoryStorer{cache.New(cache.NoExpiration, 1)}
	return memStor
}

func NewMemoryStorerWithTTL(duration time.Duration) *MemoryStorer {
	memStor := &MemoryStorer{cache.New(duration, 1)}
	return memStor
}
