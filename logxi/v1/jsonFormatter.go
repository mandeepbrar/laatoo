...
func (jf *JSONFormatter) writeString(buf bufferWriter, s string) {
	...
	if err != nil {
		InternalLog.Error(nil, InternalLogContext, "Could not json.Marshal string.", "str", s)
...
func (jf *JSONFormatter) appendValue(buf bufferWriter, val interface{}) {
	...
	default:
		...
		if err != nil {
			InternalLog.Error(nil, InternalLogContext, "Could not json.Marshal value: ", "formatter", "JSONFormatter", "err", err.Error())
			...
			if err != nil {
				...
				InternalLog.Error(nil, InternalLogContext, msg)
				buf.WriteString(`"` + msg + `"`)
				return
			}
		}
...
func (jf *JSONFormatter) Format(reqContext interface{}, loggingCtx string, writeHandler WriteHandler, level int, msg string, args []interface{}) {
	buf := pool.Get()
	defer pool.Put(buf)
	jf.format(loggingCtx, buf, level, msg, args)
	writeHandler.WriteLog(reqContext, loggingCtx, buf, level, msg, args)
}
...
// logs in production.
func (jf *JSONFormatter) LogEntry(loggingCtx string, level int, msg string, args []interface{}) map[string]interface{} {
	buf := pool.Get()
	defer pool.Put(buf)
	jf.format(loggingCtx, buf, level, msg, args)
...
func (jf *JSONFormatter) format(loggingCtx string, buf *bytes.Buffer, level int, msg string, args []interface{}) {
	const lead = `", "`
	const colon = `":"`
	...
	buf.WriteString(`", "`)
	buf.WriteString(KeyMap.LoggingContext)
	buf.WriteString(`":"`)
	buf.WriteString(loggingCtx)
