...
func (ci *frameInfo) readSource(contextLines int) error {
	...
	if err := scanner.Err(); err != nil {
		InternalLog.Warn(nil, InternalLogContext, "scanner error", "file", ci.filename, "err", err)
	}
	return nil
}
...
func (ci *frameInfo) String(color string, sourceColor string) string {
	...
	if err != nil {
		InternalLog.Warn(nil, InternalLogContext, "Could not make path relative", "path", ci.filename)
		return ""
	}
...
func parseDebugStack(stack string, skip int, ignoreRuntime bool) []*frameInfo {
	...
	for i := skip * 2; i < len(lines); i += 2 {
		...
		if err != nil {
			InternalLog.Warn(nil, InternalLogContext, "Could not parse line number", "sourceLine", sourceLine, "numstr", numstr)
			continue
		}
		ci.lineno = lineno
