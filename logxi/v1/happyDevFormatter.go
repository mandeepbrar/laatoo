...
// Format a log entry.
func (hd *HappyDevFormatter) Format(reqContext interface{}, loggingCtx string, writeHandler WriteHandler, level int, msg string, args []interface{}) {
	buf := pool.Get()
	defer pool.Put(buf)
	...
	for i := 0; i < len(args); i += 2 {
		...
		if err != nil {
			InternalLog.Error(nil, InternalLogContext, "Key is not a string.", "err", fmt.Errorf("args[%d]=%v", i, args[i]))
		} else if isReserved {
			InternalLog.Fatal(nil, InternalLogContext, "Key conflicts with reserved key. Avoiding using single rune keys.", "key", args[i].(string))
		} else {
	...
	entry := hd.jsonFormatter.LogEntry(loggingCtx, level, msg, args)
