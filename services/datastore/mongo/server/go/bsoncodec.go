...
var customStructCodec = &StructCodec{
...
var _ bsoncodec.ValueEncoder = &StructCodec{}
var _ bsoncodec.ValueDecoder = &StructCodec{}
...
func (sc *StructCodec) EncodeValue(r bsoncodec.EncodeContext, vw bsonrw.ValueWriter, val reflect.Value) error {
	...
	for _, desc := range sd.fl {
		...
		iszero := sc.isZero
		if iz, ok := encoder.(bsoncodec.CodecZeroer); ok {
	...
	if sd.inlineMap >= 0 {
		...
		return bsoncodec.defaultValueEncoders.mapEncodeValue(r, dw, rv, collisionFn)
	}
