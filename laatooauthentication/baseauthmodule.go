	...
	for _, authType := range authTypes {
		initializationErr := authType.InitializeType(
			...
			func(ctx *gin.Context) { ///  auth failed method starts
				...
				commonobjects.FireEvent(&commonobjects.Event{EVENT_LOGIN_FAIL, ctx})
				return
			...
			func(ctx *gin.Context) { ///  auth successful method starts
				...
				user := userInt.(commonobjects.Storable)
				...
				commonobjects.FireEvent(&commonobjects.Event{EVENT_JWT_TOKEN_PREPARED, ctx})
				...
				commonobjects.FireEvent(&commonobjects.Event{EVENT_LOGIN_COMPLETE, ctx})
...
func (baseAuth *BaseAuth) Logout(ctx *gin.Context) {
	ctx.Header(AuthToken, "")
	delete(ctx.Keys, "User")
	commonobjects.FireEvent(&commonobjects.Event{EVENT_LOGOUT_COMPLETE, ctx})
}
